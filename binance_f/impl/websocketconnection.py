...
def websocket_func(*args):
    ...
    connection_instance._LOGGER.info(
    ...
    connection_instance._LOGGER.info("[Sub][" + str(
        connection_instance.id) + "] Connection event loop down")
...
class WebsocketConnection:
    ...
    def __init__(self, api_key, secret_key, uri, watch_dog, request):
        ...
        self._LOGGER = logging.getLogger("binance-futures")
    ...
    def re_connect_in_delay(self, delay_in_second):
        ...
        self._LOGGER.warning("[Sub][" + str(self.id) + "] Reconnecting after "
    ...
    def re_connect(self):
        if self.delay_in_second != 0:
            ...
            self._LOGGER.warning(
                "In delay connection: " + str(self.delay_in_second))
        else:
            self.connect()

    def connect(self):
        if self.state == ConnectionState.CONNECTED:
            self._LOGGER.info("[Sub][" + str(self.id) + "] Already connected")
        else:
            self.__thread = threading.Thread(target=websocket_func,
                                             args=[self])
            self.__thread.start()
    ...
    def close(self):
        ...
        self._LOGGER.error("[Sub][" + str(self.id) + "] Closing normally")

    def on_open(self, ws):
        self._LOGGER.info("[Sub][" + str(self.id) + "] Connected to server")
        self.ws = ws
    ...
    def on_error(self, error_message):
        ...
        self._LOGGER.error("[Sub][" + str(self.id) + "] " + str(error_message))
    ...
    def on_message(self, message):
        ...
        if json_wrapper.contain_key("status") and json_wrapper.get_string(
                "status") != "ok":
            error_code = json_wrapper.get_string_or_default("err-code",
                                                            "Unknown error")
            error_msg = json_wrapper.get_string_or_default("err-msg",
                                                           "Unknown error")
            self.on_error(error_code + ": " + error_msg)
        elif json_wrapper.contain_key("err-code") and json_wrapper.get_int(
                "err-code") != 0:
            error_code = json_wrapper.get_string_or_default("err-code",
                                                            "Unknown error")
            error_msg = json_wrapper.get_string_or_default("err-msg",
                                                           "Unknown error")
            self.on_error(error_code + ": " + error_msg)
        elif json_wrapper.contain_key("result") and json_wrapper.contain_key(
                "id"):
    ...
    def __on_receive_response(self, json_wrapper):
        ...
        try:
            if self.request.update_callback is not None:
                self.request.update_callback(SubscribeMessageType.RESPONSE,
                                             res)
        except Exception as e:
    ...
    def close_on_error(self):
        if self.ws is not None:
            ...
            self._LOGGER.error("[Sub][" + str(
                self.id) + "] Connection is closing due to error")
